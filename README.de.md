...
> Die App gibt es auf Englisch und Chinesisch; beim ersten Start folgt sie deiner Systemsprache, und in den Einstellungen gibt es einen Umschalter. Deutsch ist als dritte Interface-Sprache verfügbar. Diese Seite ist eine Übersetzung der [englischen Originalfassung](README.md), die zusammen mit ihrem [chinesischen Gegenstück](README.zh-CN.md) als Referenz gepflegt wird — bei Abweichungen gilt die englische Fassung. Die verlinkten Detail-Guides unten liegen bislang nur auf Englisch und Chinesisch vor.
...
- **In einem Schritt an eine KI übergeben.** Zeitraum und Datentypen wählen; die App verpackt alles in einer modelllesbaren Form, entfernt identifizierende Details und kopiert es in die Zwischenablage.
...
Hast du die Uhr an einem Tag nicht getragen, hat der Chart an dieser Stelle eine Lücke. Hat deine Uhr etwas nie gemessen, steht im Interface „nicht verfügbar" — niemals `0`. Kein GPS-Track bedeutet keine Karte. Bei Gesundheitsdaten ist eine erfundene, glatte Kurve schlimmer als eine ehrliche Lücke.
...
| **Letzte Einträge** | Jede Schlafphase und jedes Training, jeweils im Detail zu öffnen |
...
Feldstrukturen für unbekannte Produkte, die Firmware-Version sowie unbekannte
Trainingscodes mit Anzahl. Niemals gesendet werden Konten, Tokens,
Seriennummern, Gerätekennungen, GPS, Gesundheitswerte, Rohantworten oder
lokale Pfade. Es gibt keine automatische Telemetrie und kein
...
- **Zugangsdaten** liegen im Anmeldeinformationsspeicher des Betriebssystems
  ...
  nicht in einer Klartextdatei. Auf einem Rechner ohne Schlüsselbund lässt sich
  ausdrücklich auf einen Datei- oder Umgebungsvariablen-Speicher umschalten —
  eine bewusste, dokumentierte Abstufung, siehe
  [Linux-Guide](docs/guides/linux.md) (Englisch).
...
Die verlinkte Detail-Dokumentation liegt bislang auf Englisch und vereinfachtem
Chinesisch vor, jede Seite verlinkt auf ihr Gegenstück; eine deutsche Übersetzung
davon ist noch offen. Issues und PRs sind in jeder der drei Sprachen willkommen.
Bevor du etwas änderst, lies die Liste „unverified" im Architektur-Dokument —
dieses Projekt hat einen expliziten Standard dafür, was als gesicherte
Tatsache gilt.
