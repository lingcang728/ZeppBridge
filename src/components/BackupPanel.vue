...
const messages = defineMessages(
  ...
  {
    title: 'Datenbank-Sicherungen und Wiederherstellung',
    intro1a: 'Eine Sicherung ist eine vollständige Kopie der gesamten ',
    intro1b: '-Datei. Sie bleibt auf diesem Rechner und wird niemals irgendwohin hochgeladen. Vor einem Datenbank-Upgrade wird automatisch eine erstellt, und du kannst jederzeit auch von Hand eine anlegen.',
    compareLead: 'Drei Dinge heißen hier „Export", sind aber nicht dasselbe: ',
    compareExchange: ' ist der Datenaustausch für andere Tools und enthält nur den gewählten Zeitraum;',
    compareSnapshotName: 'eine Datenbank-Sicherung',
    compareSnapshot: ' ist eine vollständige Kopie der Datenbank für den Ernstfall, die nur ZeppBridge selbst wieder einlesen kann;',
    comparePackName: 'ein KI-Datenpaket',
    comparePack: ' ist Material, das du bewusst auswählst und anonymisierst, um es an ein externes Modell zu geben. Nur eine Sicherung kann die Datenbank wieder in ihren vorherigen Zustand bringen.',

    pendingTitle: 'Eine Wiederherstellung ist eingeplant',
    pendingBodyA: (stagedAt: string) => `Eingeplant am ${stagedAt}. Die Datenbank wird beim `,
    pendingNextStart: 'nächsten Start',
    pendingBodyB: ' ersetzt. Die aktuelle Datenbank ist bereits als Rückfallpunkt gesichert, falls dabei etwas schiefgeht.',
    cancelRestore: 'Wiederherstellung abbrechen',

    creating: 'Wird erstellt…',
    createSnapshot: 'Sicherung erstellen',
    refreshList: 'Aktualisieren',
    noSnapshots: 'Noch keine Sicherungen vorhanden.',

    pinned: 'Behalten',
    metaLine: (size: string, appVersion: string, schemaVersion: number) =>
      `${size} · App ${appVersion} · Schema ${schemaVersion}`,
    coverage: (from: string, to: string) => ` · Messwerte ${from} ~ ${to}`,
    noSamples: ' · keine Gesundheitsdaten in dieser Sicherung',
    verifyFailed: (problem: string) => `Überprüfung fehlgeschlagen: ${problem}`,
    problemFileMissing: 'Die Sicherungsdatei liegt nicht mehr im Sicherungsordner',
    problemSizeMismatch: 'Die Größe der Sicherungsdatei stimmt nicht mit dem Manifest überein — sie könnte beschädigt sein',
    problemSha256Mismatch:
      'Der SHA-256-Wert der Sicherungsdatei stimmt nicht mit dem Manifest überein — sie könnte beschädigt oder verändert worden sein',
    problemIntegrityFailed: 'Die Sicherungsdatei hat die SQLite-Integritätsprüfung nicht bestanden',
    problemUnknown: 'Diese Sicherung hat die Überprüfung nicht bestanden, und es wurde kein Grund aufgezeichnet.',
    verifyPassed: 'Gerade erneut überprüft: Datei, Größe, SHA-256 und Integrität stimmen alle überein.',
    integrityOk: (sha: string) => `Integritätsprüfung bei der Erstellung bestanden · SHA-256 ${sha}…`,
    integrityBad: 'Die Integritätsprüfung ist bei der Erstellung fehlgeschlagen. Nicht für eine Wiederherstellung verwenden.',
    verifyAgain: 'Erneut überprüfen',
    unpin: 'Nicht mehr behalten',
    pin: 'Behalten',
    restoreToThis: 'Auf diese wiederherstellen',

    previewTitle: 'Vorschau der Wiederherstellung',
    compatibilityUnknown: 'Kompatibilität unbekannt.',
    colContent: 'Inhalt',
    colBackup: 'In der Sicherung',
    colCurrent: 'Aktuell',
    colDelta: 'Differenz',
    previewNote: 'Zeilen mit negativer Differenz enthalten nach der Wiederherstellung entsprechend weniger Datensätze. Eine Wiederherstellung ruft nie erneut Daten aus der Cloud ab — falls du diese Daten noch brauchst, synchronisiere danach noch einmal.',
    staging: 'Wird eingeplant…',
    stageRestore: 'Wiederherstellung einplanen (wirkt beim nächsten Start)',
    cancel: 'Abbrechen',

    listFailed: 'Die Liste der Sicherungen konnte nicht gelesen werden',
    created: (size: string) => `Sicherung erstellt: ${size}, Integritätsprüfung bestanden.`,
    createFailed: 'Die Sicherung konnte nicht erstellt werden',
    verifyError: 'Überprüfung fehlgeschlagen',
    pinFailed: 'Die Markierung „Behalten" konnte nicht geändert werden',
    previewFailed: 'Die Vorschau der Wiederherstellung konnte nicht erstellt werden',
    staged: 'Die Wiederherstellung ist eingeplant. In diesem Durchlauf ändert sich nichts — die Datenbank wird erst beim nächsten Start von ZeppBridge ersetzt.',
    stageFailed: 'Die Wiederherstellung konnte nicht eingeplant werden',
    cancelled: 'Die eingeplante Wiederherstellung wurde abgebrochen. Die Datenbank bleibt unverändert.',
    cancelFailed: 'Die Wiederherstellung konnte nicht abgebrochen werden',

    kind: {
      manual: 'manuell',
      pre_migration: 'vor Upgrade',
      pre_restore: 'Rückfallpunkt',
    },
    compatibility: {
      same_schema: 'Die Sicherung hat dieselbe Schema-Version wie diese App und kann direkt wiederhergestellt werden.',
      older_schema_will_migrate: 'Die Sicherung stammt aus einer älteren Schema-Version. Nach der Wiederherstellung wird sie beim nächsten Start automatisch aktualisiert.',
      future_schema_refused: 'Die Sicherung stammt aus einer neueren App-Version, deren Struktur diese App nicht lesen kann, und kann deshalb nicht wiederhergestellt werden.',
    },
    table: {
      raw_records: 'Rohdaten',
      workouts: 'Trainings',
      daily_summaries: 'Tagesübersichten',
      metric_samples: 'Messwerte',
