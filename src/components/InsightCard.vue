...
const messages = defineMessages(
  ...
  },
  {
    ...
    unsupportedWorkoutType: 'Erkenntnisse für diese Trainingsart werden noch nicht unterstützt. Die erste Version deckt nur Laufen ab, weil das bisher mit echten Daten überprüft wurde. Alle anderen Trainingsarten werden weiterhin normal angezeigt, korrigiert und exportiert.',
    handoff: 'KI analysieren lassen',
    reading: 'Lokale Aufzeichnungen werden gelesen…',
    comparedTo: (count: number) => `Im Vergleich zu deinen eigenen ${count} letzten Läufen mit ähnlicher Distanz:`,
    noComparison: 'Noch nicht genug vergleichbare Historie, daher zeigt dieser Lauf nur seine Werte ohne Vergleich.',
    baselinePrefix: (value: string, delta: string) => `Basiswert ${value} · ${delta}`,
    driftTitle: 'Erste Hälfte vs. zweite',
    driftSub: 'Teilt dieses Training zeitlich in zwei Hälften und vergleicht, wie viele Herzschläge dasselbe Tempo gekostet hat.',
    driftFirst: 'Erste Hälfte',
    driftSecond: 'Zweite Hälfte',
    driftPerBeat: (metres: string) => `${metres} m/Schlag`,
    driftHrSpeed: (hr: number, pace: string) => `${hr} bpm · ${pace}`,
    driftDelta: (percent: string) => `${percent}%`,
    driftRising: 'Das gleiche Tempo zu halten hat in der zweiten Hälfte mehr Herzschläge gekostet.',
    driftFlat: 'Die beiden Hälften sind im Wesentlichen gleich.',
    driftFalling: 'Jeder Herzschlag hat in der zweiten Hälfte weiter getragen.',
    driftNote: 'Das vergleicht das Training nur mit sich selbst, nie mit jemand anderem. Ampeln, Steigungen, Intervalle und GPS-Drift verfälschen das alles, daher wird kein Wert angezeigt, wenn das Tempo nicht konstant war.',
    driftUnavailable: (code: string) => ({
      too_short: 'Zu kurz zum Teilen. In den ersten zehn Minuten steigt die Herzfrequenz meist noch an, ein Vergleich mit der zweiten Hälfte würde also das Aufwärmen messen, nicht die Drift.',
      pace_too_variable: 'Das Tempo hat zu stark geschwankt (Intervalle, Ampeln oder Steigungen sehen alle so aus), daher sind die beiden Hälften nicht vergleichbar und es wird kein Wert angezeigt.',
      not_enough_samples: 'Nicht genug Herzfrequenz- und Tempo-Einzelwerte in diesem Training, um es zu teilen.',
      unsupported_workout_type: 'Der Vergleich erste/zweite Hälfte deckt vorerst nur Laufen ab. Gehen und Radfahren liefern zwar genug Messwerte, aber die Schwellenwerte wurden noch nicht mit echten Daten überprüft.',
    } as Record<string, string | undefined>)[code] ?? 'Dieses Training lässt sich nicht teilen.',
    baselineSummary: 'Woher der Basiswert kommt',
    baselineRule: (days: number, tolerance: number | null | undefined, min: number, max: number) =>
      `Die Regel: Läufe derselben Art aus den letzten ${days} Tagen, deren Distanz innerhalb von ±${tolerance ?? '—'}% dieses Laufs liegt, mindestens ${min} und höchstens ${max} davon.`,
    excludedPrefix: 'Ausgeschlossen: ',
    excludedItem: (label: string, count: number) => `${label} ×${count} `,
    footnote: 'Jede Schlussfolgerung hier vergleicht dich nur mit deiner eigenen Historie — nie mit einem Bevölkerungsdurchschnitt — und nichts davon ist eine medizinische Einschätzung. Fehlende Daten werden als „Nicht angegeben" angezeigt statt mit einer Null aufgefüllt.',
    ...
    exclusion: {
      ...
      beyond_max_samples: 'über dem Stichprobenlimit',
    },
  },
);
