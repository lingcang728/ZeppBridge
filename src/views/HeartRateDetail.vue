...
const messages = defineMessages(
  ...
  },
  {
    ...
    title: 'Herzfrequenz',
    intro: 'Der Herzfrequenzverlauf des heutigen Tages sowie Ruheherzfrequenz und HRV im Tagesvergleich. Abschnitte ohne Messwerte bleiben leer und werden nicht mit einer Null aufgefüllt.',
    rangeAria: 'Trend-Zeitraum',
    desktopOnly: 'Nutze die Desktop-App. Diese Browser-Vorschau liest keine Kontodaten.',
    ...
    daySub: 'Einzelne Messwerte in zeitlicher Reihenfolge',
    statLatest: 'Aktuell',
    statAverage: 'Ø',
    statLowest: 'Min',
    statHighest: 'Max',
    chartAria: 'Herzfrequenzverlauf der letzten 24 Stunden',
    noSamples: 'Keine Herzfrequenzmessungen in den letzten 24 Stunden, deshalb gibt es hier keine Kurve.',
    bpmTooltip: (clock: string, value: number) => `${clock}　<b>${value}</b> bpm`,
    restingLabel: 'Ruheherzfrequenz',
    restingHint: 'Die Uhr liefert einmal täglich einen Wert; je stabiler, desto besser',
    hrvHint: 'Einzelne HRV-Messwerte, täglich gemittelt',
    rmssdHint: 'Ein anderes HRV-Mass, nicht derselbe Wert wie oben',
    emptyCard: 'In diesem Zeitraum wurden keine Daten aufgezeichnet.',
    dailyMaxTitle: 'Tägliche Spitzenherzfrequenz (Rohdaten auf diesem Rechner)',
    dailyMaxSub: 'Die Zepp-App filtert ihren Tageshöchstwert, hier wird nicht gefiltert — dass die beiden Zahlen abweichen, ist normal.',
    dailyMaxAria: 'Trend der täglichen Spitzenherzfrequenz',
    dailyMaxNone: 'Für diesen Zeitraum liegen auf diesem Rechner keine Herzfrequenzmessungen vor, daher gibt es keinen Spitzenwert zum Vergleich.',
    dailyMaxSparse: (days: number) =>
      `An ${days} dieser Tage gibt es nur sehr wenige Messwerte (weniger als 60). An diesen Tagen ist der „Höchstwert" nur das Maximum dieser wenigen Punkte, nicht der tatsächliche Tageshöchstwert — sie werden als hohle Marker dargestellt.`,
    ...
    dailyMaxNote: 'Hier werden nur die auf diesem Rechner gespeicherten Rohmesswerte verwendet. Der Tageshöchstwert von Zepp wird uns nie übermittelt (der device_max_hr in der Bibliothek ist die eingestellte Maximalherzfrequenz für die Zonengrenzen, kein gemessener Höchstwert) — deshalb gibt es hier nichts zum direkten Vergleich. Öffne die Zepp-App, um diesen Tageswert nachzuschauen.',
  },
);
