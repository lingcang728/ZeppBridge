...
const messages = defineMessages(
  ...
  },
  {
    ...
    loadFailed: 'Datenzustand konnte nicht gelesen werden',
    retry: 'Erneut versuchen',
    noRecords: 'Noch keine Datensätze',
    timeUnknown: 'Zeitpunkt unbekannt',
    notProvided: 'Nicht verfügbar',
    backToSettings: 'Zurück zu den Einstellungen',
    eyebrow: 'Datenzustand',
    title: 'Datenzustand-Check',
    intro: 'Für jeden Datenstrom: wie weit er beim Abruf aus der Cloud, beim Verarbeiten und beim lokalen Schreiben gekommen ist; welche Daten er abdeckt; und woher er stammt. Fehlend ist fehlend — nie mit einer Null aufgefüllt.',
    rangeAria: 'Abdeckungszeitraum',
    loadingAria: 'Datenzustand wird gelesen',
    replayInProgress: 'Lokale Rohdaten werden mit dem neuen Parser neu verarbeitet. Cloud-Synchronisierungen treten dabei zurück und wiederholen sich automatisch; das ist kein Fehler.',

    timingsTitle: 'Drei verschiedene „letzte Male"',
    timingCloud: 'Zuletzt aus der Cloud abgerufen',
    timingCloudNote: 'Noch kein Ergebnis',
    timingReplay: 'Letzte lokale Neuverarbeitung',
    timingReplayNote: 'Liest lokale Rohdaten mit dem aktuellen Parser erneut. Kein Netzwerkzugriff, und der obige Zeitpunkt wird nicht überschrieben.',
    timingManual: 'Letzte manuelle Neuverarbeitung',
    timingManualNote: 'Die, auf die du selbst geklickt hast',
    timingNewest: 'Neuester Gesundheitswert',
    timingNewestNote: 'Wann der Datensatz selbst auf der Uhr entstanden ist',

    dbTitle: 'Lokale Datenbank',
    dbSize: 'Dateigrösse',
    dbRaw: 'Rohdaten',
    dbCanonical: 'Normalisierte Datensätze',
    dbPending: 'Ausstehende Normalisierung',
    dbSchema: 'Schema-Version',
    dbNormalizer: 'Parser-Revision',
    integrityPassed: 'bestanden',
    integrityFailed: (detail: string) => `nicht bestanden (${detail})`,
    integrityDetailBelow: 'Details siehe unten',
    integrityLine: (verdict: string, checkedAt: string) => `Integritätsprüfung: ${verdict} · ${checkedAt}`,
    integrityNeverRun: 'Es wurde noch keine Integritätsprüfung ausgeführt. Sie durchsucht die gesamte Datenbank, was bei einer grossen Datenbank etwas dauert, daher läuft sie nur auf ausdrückliche Anforderung.',

    streamsTitle: 'Wie weit jeder Datenstrom gekommen ist',
    streamsNote: 'Abrufen, Verarbeiten und Schreiben sind drei Dinge, die jeweils für sich fehlschlagen können. Zu einem einzigen roten Punkt zusammengefasst liesse sich nicht sagen, ob erneut versucht, neu verbunden werden muss, oder ob dieses Konto diesen Datenstrom schlicht nicht hat.',
    stageFetch: 'Abrufen',
    stageParse: 'Verarbeiten',
    stageWrite: 'Schreiben',
    stageLine: (stage: string, state: string) => `${stage}: ${state}`,
    factRaw: 'Rohdaten',
    factCanonical: 'Normalisierte Datensätze',
    factSources: 'Quellen',
    factObservedDays: 'Beobachtete Tage',
    days: (count: number) => `${count} Tage`,
    gapExamples: (dates: string) => `Lücken unter anderem: ${dates}`,
    gapMore: ' und weitere',
    period: '.',
    latestObserved: (date: string) => `Zuletzt am ${date}.`,
    noRecordsYet: 'Noch keine Datensätze',
    sourceSeparator: ', ',

    occasionalTitle: 'Messwerte, die nur gelegentlich auftauchen',
    occasionalNote: 'Messwerte wie VO₂max und Laktatschwelle werden bewusst nicht täglich gemeldet. Dieser Abschnitt zeigt die beobachteten Tage und den letzten davon und zählt nie tägliche Lücken — normale Spärlichkeit rot einzufärben wäre irreführend.',
    occasionalLine: (records: string, days: number) => `${records} Datensätze · beobachtet an ${days} Tagen`,
    occasionalLatest: (date: string) => `zuletzt am ${date}`,
    occasionalNone: 'In diesem Zeitraum nichts beobachtet',

    actionsTitle: 'Was du tun kannst',
    actionRunning: 'Wird ausgeführt …',
    actionRun: 'Ausführen',
    confirmDestructive: (label: string, reason: string) => `${label}: ${reason}\nFortfahren?`,
    actionSynced: 'Synchronisierung ausgeführt, Status aktualisiert.',
    actionReplayed: (count: string) => `Lokale Rohdaten mit dem aktuellen Parser neu verarbeitet (${count} abgeleitete Datensätze). Der Cloud-Synchronisierungszeitpunkt wurde nicht überschrieben.`,
    actionIntegrityOk: 'Die Datenbank hat die Integritätsprüfung bestanden.',
    actionIntegrityFailed: (detail: string) => `Die Datenbank hat die Integritätsprüfung nicht bestanden: ${detail}`,
    actionIntegrityFallback: 'Datenordner sichern und erneut synchronisieren',
    actionFolderOpened: 'Datenordner geöffnet.',
    actionReconnect: 'In den Einstellungen das Zepp-Konto erneut verbinden.',
    actionFailed: (label: string) => `${label} fehlgeschlagen`,

    coveragePerEvent: 'Ereignisbasiert erzeugt: Kein Datensatz bedeutet, dass damals nichts passiert ist, nicht dass etwas fehlt.',
    coverageOccasional: 'Die Uhr meldet das nur gelegentlich; leere Tage sind normal und bedeuten keinen Datenverlust.',
    coverageNoData: 'Für diesen Zeitraum liegen noch keine lokalen Daten vor. Zuerst synchronisieren.',
    coverageNoGaps: 'Seit dem ersten Tag mit Daten wurden keine Lücken beobachtet.',
    coverageGaps: (days: number) =>
      `Seit dem ersten Tag mit Daten wurden an ${days} Tagen keine Daten beobachtet. Uhr nicht getragen, nicht synchronisiert oder eine leere Antwort der Cloud verursachen alle Lücken.`,

    action: {
      reauth: { label: 'Zepp-Konto erneut verbinden', reason: 'Einige Datenströme können nicht abgerufen werden, weil die Anmeldedaten abgelaufen sind.' },
      reprocess: { label: 'Lokale Rohdaten mit dem aktuellen Parser neu verarbeiten', reason: '' },
      sync_retry: { label: 'Erneut synchronisieren', reason: 'Einige Datenströme konnten beim letzten Mal nicht aus der Cloud abgerufen werden.' },
      sync_first: { label: 'Erste Synchronisierung ausführen', reason: 'Für diesen Rechner liegt noch keine erfolgreiche Cloud-Synchronisierung vor.' },
      integrity_check: { label: 'Datenbankintegrität prüfen', reason: 'Führt eine SQLite-Integritätsprüfung über die gesamte Datenbank aus; bei einer grossen Datenbank dauert das etwas.' },
      open_data_folder: { label: 'Datenordner öffnen', reason: 'Die lokale Datenbank, Backups und Exporte liegen alle hier.' },
    },
    reprocessReason: (pending: number) =>
      `${pending} gespeicherte Rohdaten haben noch keinen normalisierten Datensatz erzeugt. Eine Neuverarbeitung greift nicht auf das Netzwerk zu und überschreibt den Cloud-Synchronisierungszeitpunkt nicht.`,

    cadence: {
      continuous: 'mehrmals täglich',
      daily: 'einmal täglich',
      nightly: 'einmal pro Nacht',
      per_event: 'nur wenn es passiert',
      occasional: 'nur gelegentlich',
    },
    stage: { ok: 'OK', failed: 'fehlgeschlagen', never: 'noch nie' },
    errorKind: {
      network: 'Cloud nicht erreichbar',
      auth: 'Konto muss erneut verbunden werden',
      not_available: 'Dieses Konto hat diesen Datenstrom nicht',
      unrecognized_payload: 'Rohdaten erhalten, aber nicht verstanden',
      storage: 'Schreiben in die lokale Datenbank fehlgeschlagen',
      busy: 'Ein anderer Vorgang hat gerade geschrieben, dieser ist zurückgetreten',
      cancelled: 'abgebrochen',
      unknown: 'nicht klassifizierter Fehler',
    },
    source: {
      device: 'Einzelgerät',
      user_fused: 'nutzerübergreifend zusammengeführt',
