...
Answer in Markdown.`,
  ...
  },
  {
    ...
    backToRecent: 'Zurück zu den letzten Aufzeichnungen',
    loadFailedTitle: 'Dieses Training konnte nicht gelesen werden',
    loadFailed: 'Trainingsdetails sind gerade nicht verfügbar',
    retry: 'Erneut versuchen',
    notFoundTitle: 'Dieses Training ist nicht vorhanden',
    notFoundMessage: 'Es wurde möglicherweise bereinigt oder noch nicht auf diesen Rechner synchronisiert.',
    insightFailed: 'Für dieses Training konnte keine Auswertung erstellt werden',
    thisWorkout: 'Training',
    aiPrompt: (label: string) => `Du bist ein Sportanalyst. Unten steht der vollständige Datensatz eines ${label} von mir, aus der lokalen ZeppBridge-Datenbank und anonymisiert.
Analysiere dieses Training ausschliesslich anhand der Fakten in diesem Datensatz: die Intensität, wie Tempo und Herzfrequenz zusammenhängen, ob es einen deutlichen Tempoabfall oder einen auffälligen Abschnitt gibt, und was ich konkret beim nächsten Mal anders machen sollte.
...
Antworte in Markdown.`,
    ...
    attachmentOpened: (provider: string) =>
      `Das Datenpaket wurde auf den Desktop geschrieben (zeppbridge-ai-handoff.json) — in ${provider} hineinziehen. Der Prompt liegt in der Zwischenablage.`,
    attachmentNotOpened: (provider: string) =>
      `Das Datenpaket wurde auf den Desktop geschrieben (zeppbridge-ai-handoff.json). Der Prompt liegt in der Zwischenablage; ${provider} selbst öffnen.`,
    copiedAndOpened: (provider: string) => `Anonymisierte Daten dieses Trainings kopiert und ${provider} geöffnet. Dort einfügen.`,
    copiedOnly: (provider: string) => `Anonymisierte Daten dieses Trainings kopiert. ${provider} selbst öffnen und dort einfügen.`,
    noCorrection: 'Keine Korrektur',
    deviceNameMissing: 'Gerätename nicht verfügbar',
    notFetchedYet: 'Noch nicht abgerufen',
    timeUnknown: 'Zeitpunkt unbekannt',
    overrideSaved: 'Lokale Trainingsart-Korrektur gespeichert.',
    overrideCleared: 'Korrektur entfernt. Zurück zur Erkennung von ZeppBridge.',
    overrideFailed: 'Trainingsart-Korrektur konnte nicht gespeichert werden',
    copied: (format: string) => `${format}-Daten in die Zwischenablage kopiert.`,
    copyFailed: 'Dieser Datensatz konnte nicht kopiert werden',

    metricDistance: 'Distanz',
    metricDuration: 'Bewegungszeit',
    metricAvgHr: 'Ø Herzfrequenz',
    metricAvgPace: 'Ø Tempo',
    metricAscent: 'Aufstieg',
    ...
    chartAria: (title: string) => `${title}-Verlauf`,

    decodedRoutePoints: 'GPS-Trackpunkte',
    decodedSamples: 'Zeitreihen-Messpunkte',
    ...
    decodedDescent: 'Abstieg',
    decodedMaxHr: 'Max. Herzfrequenz',
    decodedAvgPower: 'Ø Leistung',
    decodedMaxPower: 'Max. Leistung',
    decodedGroundContact: 'Ø Bodenkontaktzeit',
    decodedVerticalOscillation: 'Ø vertikale Oszillation',
    decodedVerticalRatio: 'Vertikales Verhältnis',
    decodedBestEquivalentPace: 'Bestes Äquivalenztempo',
    ...
    zeppBridgeMatch: (label: string) => `ZeppBridge erkennt: ${label}`,
    customName: (code: string, name: string) => `Dein Name für Code ${code}: ${name}`,
    myCorrection: 'Meine Korrektur',
    correctionAria: 'Meine Korrektur für diese Trainingsart',
    metricListAria: 'Zusammenfassung der Trainingsleistung',

    routeAria: 'Vollständiger GPS-Track',
    routeTitle: 'Vollständiger GPS-Track',
    routeNote: 'Lokal gezeichnet · keine Kartenkacheln angefragt',
    routeSvgAria: 'Lokaler GPS-Track, eingefärbt nach Zeit und nächstliegendem Tempo-Messpunkt',
    routeLegendPace: (count: number) => `${count} gültige Tempopunkte · P10–P90`,
    routeLegendNoPace: 'Weniger als 3 gültige Tempopunkte · nicht nach Geschwindigkeit eingefärbt',
    legendFast: 'Schnell',
    legendSteady: 'Gleichmässig',
    legendWarm: 'Langsamer',
    legendSlow: 'Langsam',
    routeEmptyTitle: 'Kein nutzbarer Track',
    routeEmptyBody: 'Dieser Datensatz enthält nicht genügend GPS-Punkte, daher wird keine Route gezeichnet.',
    chartsEmptyTitle: 'Keine Einzelpunkt-Kurven',
    chartsEmptyBody: 'Für dieses Training wurden keine Herzfrequenz-, Tempo-, Höhen- oder Schrittfrequenz-Reihen synchronisiert.',

    hrZonesAria: 'Herzfrequenzzonen',
    hrZonesTitle: 'Herzfrequenzzonen',
    hrZonesNote: 'Die Zonengrenzen stammen aus deinen eigenen Einstellungen auf der Uhr und werden von Zepp zusammen mit diesem Training übermittelt; ZeppBridge teilt sie nicht neu ein. Die Trainingsstatus-Seite verwendet ein separates, selbst gewähltes Modell, daher stimmen die beiden Zahlensätze nicht überein.',
    hrZoneBelow: (upper: number) => `Unter ${upper}`,
    hrZoneBetween: (low: number, high: number) => `${low}–${high}`,
    hrZoneShare: (percent: string) => `${percent}%`,
    hrZoneTotal: (duration: string) => `${duration} mit Herzfrequenz`,
    hrZoneBarAria: 'Zeitanteil je Herzfrequenzzone',
    decodedAria: 'Dekodierte Werte',
    decodedTitle: 'Dekodierte Werte',
    decodedNote: 'Die Zusammenfassung wird nur aus gültigen Messpunkten dieses Datensatzes berechnet; auffällige Ausreisser werden ignoriert.',

    exportAria: 'Export und Teilen',
    exportTitle: 'Export und Teilen',
    exportSub: 'Kopiert die lokal dekodierten strukturierten Daten. Es wird kein Kartendienst kontaktiert.',
    ...
    handoffSub: 'Kopiert die anonymisierten Daten nur dieses einen Trainings, plus den Prompt, und öffnet die gewählte KI-Seite. Tagesweise Datenströme wie Schlaf und Schritte bleiben aussen vor.',
    handoffTarget: 'Zieltool',
    handoffTargetAria: 'An welches KI-Tool übergeben werden soll',
    ...
    provenanceScope: 'Umfang',
    provenanceSynced: 'Zuletzt synchronisiert',
    provenanceRecordId: 'Datensatz-ID',
    provenanceDevice: 'Gerät',
    pageFoot: 'Auf diesem Rechner dekodiert. Der Track wird auf einem lokalen Canvas gezeichnet und nie an einen Kartendienst gesendet.',
  },
);
