...
如果你需要我提供什么信息（比如我用的是哪个客户端、文件放在哪），直接问我。`,
    ...
    capabilityFoodHistoryHint: '云端有饮食记录，但本机尚未收录。若记录早于最近一次增量同步，请运行覆盖该日期的历史同步；仍未出现时再反馈报文格式。',
...
If you need anything from me (which client I use, where the file lives), just ask.`,
    ...
    capabilityFoodHistoryHint: 'Food records exist in the cloud but are not stored locally. If they predate the incremental sync window, run a history sync covering those dates. If they still do not appear, report the payload format.',
...
Si necesitas algo de mí (qué cliente uso, dónde está el archivo), pregúntame.`,
    ...
    capabilityFoodHistoryHint: 'Hay registros de comidas en la nube, pero aún no están guardados localmente. Si son anteriores al periodo de sincronización incremental, sincroniza el historial que cubra esas fechas. Si siguen sin aparecer, informa del formato de los datos.',
