...
const messages = defineMessages(
  ...
  {
    typeSteps: 'Schritte',
    typeDailyActivity: 'Tagesaktivität',
    typeWorkouts: 'Trainings',
    typeSleep: 'Schlaf',
    typeHeartRate: 'Herzfrequenz',
    typeSpo2: 'Sauerstoffsättigung',
    typeStress: 'Stress',
    ...
    detailSummaryHint: 'Herzfrequenz stündlich zusammengefasst, sekundengenaue Trainingsreihen werden weggelassen. Strukturierte Messwerte bleiben vollständig, die Grösse eignet sich zur Weitergabe an eine KI.',
    detailFull: 'Vollständig',
    detailFullHint: 'Enthält sekundengenaue Trainingsreihen und einzelne Herzfrequenzwerte. Umfangreich und für die Archivierung gedacht.',
    scopeConflict: 'Ein Datumsbereich und ein einzelnes Training schliessen sich gegenseitig aus. Wähle eines von beiden.',
    noDataTypes: 'Wähle mindestens einen Datentyp aus.',
    invalidDates: 'Wähle ein gültiges Start- und Enddatum aus.',
    endBeforeStart: 'Das Enddatum darf nicht vor dem Startdatum liegen.',
    rangeTooLong: (days: number) => `Ein Export umfasst höchstens ${days} Tage. Für einen längeren Zeitraum den Datenbank-Snapshot in den Einstellungen verwenden.`,
    nothingToExport: 'In diesem Zeitraum gibt es nichts zu exportieren.',
    jsonTooLarge: 'Die JSON-Datei ist grösser als 1 MB. Nutze stattdessen „Datei speichern".',
    copied: (count: number) => `${count} normalisierte Datensätze kopiert.`,
    copyFailed: 'JSON konnte nicht kopiert werden',
    saveJsonTitle: 'ZeppBridge JSON speichern',
    saveCsvTitle: 'ZeppBridge CSV speichern (Übersichtstabelle)',
    ...
    gpxFilter: 'GPX-Track',
    fitFilter: 'FIT-Aktivitätsdateien',
    ...
    savedFiles: (files: number, count: number, unit: string) =>
      `${files} FIT-Dateien gespeichert, insgesamt ${count} ${unit}.`,
    saveFailed: (format: string) => `${format} konnte nicht gespeichert werden`,
    feedUpdated: (count: number) => `Die lokale KI-Datenquelle enthält jetzt ${count} Datensätze.`,
    feedFailed: 'Lokale KI-Datenquelle konnte nicht aktualisiert werden',
  },
);
